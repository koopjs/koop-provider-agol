...
ImportService.prototype._handlePage = function (error, json) {
  ...
  esriToGeojson(self.fields, json, function (err, geojson) {
    ...
    self._updateCount(geojson.features.length)
    if (self.writeToDb) self._insertIntoDb(geojson)
  })
}

/**
 * Update the info doc with the total count of imported features
 *
 * @param {integer} count - the number of features in a page
 * @private
 */
ImportService.prototype._updateCount = function (count) {
  var self = this
  self.count += count
  self.cache.getInfo(self.key, function (err, info) {
    if (err) self.log.error(err)
    info.importCount = self.count
