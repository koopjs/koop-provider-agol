...
function ImportService (options) {
  ...
  self.count = 0
  self.start = Date.now()
  self.featureService = Utils.initFeatureService(options.server, { layer: self.layer, logger: self.log })
...
ImportService.prototype._prepareFc = function () {
  ...
  self.uploadPath = path.join(filePath, self.item + '_' + self.layer + '.geojson')
  var upload = self.upload = self.files.createWriteStream(self.uploadPath, {
    ContentType: 'application/json',
    Metadata: {
      // Note to self, AWS will mangle these into lower case
      upload_start: new Date(self.start).toISOString()
    }
  })
...
ImportService.prototype._completeJob = function () {
  var self = this
  if (self.count < 1) return self.emit('error', new Error('No features in service'))
  var layerInfo = self.featureService.layerInfo()
  ...
  self.upload.on('finish', function () {
    self.cache.getInfo(self.key, function (err, info) {
      ...
      layerInfo.editingInfo = layerInfo.editingInfo || {}

      var hash = self.sha.digest('hex')
      var updated = (info.sha1 !== hash) || info.itemDirty || (info.lastEditDate !== layerInfo.editingInfo.lastEditDate)
      info.lastEditDate = layerInfo.editingInfo.lastEditDate
      info.sha1 = hash
      ...
      self.emit('done', {updated: updated, info: info, file: self.uploadPath, retrieved: self.start, item: self.item, layer: self.layer})
    })
  })
}
