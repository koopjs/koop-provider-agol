...
ImportService.prototype.run = function () {
  ...
  self.featureService.info(function (err, fsInfo) {
    ...
    self.featureService.pages(function (err, pages) {
      if (err) return self.emit('error', err)
      if (pages.length < 1) return self.emit('error', new Error('Service has nothing to page over'))
...
ImportService.prototype._prepareFc = function () {
  ...
  self.upload = self.files.createWriteStream(uploadPath)
...
ImportService.prototype._handlePage = function (error, json) {
  ...
  try {
    var geojson = esriToGeojson(json, self.fields)
    ...
    self.emit('error', new Error('Error translating features to geojson'))
  }
}
