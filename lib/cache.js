...
Cache.prototype.updateFeatureService = function (options, callback) {
  ...
  self.cache.getInfo(key, function (err, info) {
    ...
    if (info.importEnqueued < Date.now() + 24 * 60 * 60 * 1000) return callback()
    info.importEnqueued = Date.now()
    self.cache.updateInfo(key, info, function (err) {
      if (err) return callback(err)
      enqueue(self.featureQueue, options)
      callback(null, info)
    })
...
Cache.prototype._prepareInsert = function (options) {
  var table = {
    ...
    info: {
      ...
      item_title: options.itemInfo.title,
      _indexFields: false,
