...
Cache.prototype.insertFeatureService = function (options, callback) {
  var self = this
  console.log('dis be the job', options.key, options.optionKey)
  self._prepareInsert(options, function (err, table) {
    ...
    self.insert(options.item, options.layer, table, function (err) {
      if (err) return callback(err)
      console.log('feature request time@!')
      fR.submit(callback)
    })
  })
}
...
Cache.prototype._prepareInsert = function (options, callback) {
  ...
  service.info(function (err, serviceInfo) {
    ...
    service.layerInfo(function (err, layerInfo) {
      if (err) return callback(err)
      if (!options.itemInfo.url) {
        self.log.error('Item has no url', options)
        return callback(new Error('Item has no url'))
      }
      if (!layerInfo.fields) {
        self.log.error('No fields in response at', options.itemInfo.url)
        layerInfo.fields = []
      }
      var table = {
