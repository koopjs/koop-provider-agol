  ...
  self.getInfo(key, function (err, info) {
    ...
    self.updateInfo(key, info, function (err) {
      ...
      enqueue(self.featureQueue, {
        item: options.item,
        layer: options.layer,
        // itemTitle is a new field, so just backfill with info.name for backwards compatibility
        itemTitle: info.itemTitle || info.name,
        url: info.url,
        id: options.id
      })
      callback(null, info)
...
function enqueue (queue, options) {
  queue.enqueue('agol', 'importService', [{
    ...
    item: options.item,
    layer: options.layer,
    itemTitle: options.itemTitle || options.itemInfo.title,
    hostId: options.id,
