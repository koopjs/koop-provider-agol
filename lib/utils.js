...
Utils.createExportOptions = function (req, table, job, format) {
  ...
  var dirname = Utils.createFilePath(req.optionKey, job || req.params)
  return {
    item: (job && job.item) || req.params.item,
    layer: (job && job.layer) || req.params.layer,
    source: path.join(dirname, geojson),
