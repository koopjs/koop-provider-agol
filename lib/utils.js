...
Utils.createName = function (item, service, layerId) {
  ...
  var numLayers = service.layers.length + service.tables.length
  var singleLayerItem = url_parts[url_parts.length - 1].match(/\d+/) || numLayers < 2
  var name = singleLayerItem ? item.title : service.layers[layerId].name
  // replace "/", ",", "&", "|", "(", ")"
  var cleansedName = name.replace(/\/|,|&|\||\(|\)/g, '').replace(/\s/g, '_')
  return cleansedName
}
 ...
 *
 * @param {object} req - the incoming request object
 * @param {string} table - the table tog et data from
...
Utils.createGeohashOptions = function (req) {
  return {
    filePath: ['latest', 'files', req.optionKey].join('/'),
    fileName: req.optionKey + '.geohash.json',
    query: req.query,
    infoKey: Utils.createTableKey(req.params)
  }
}
...
Utils.processingTime = function (info, key) {
  ...
  var seconds = (Date.now() - start) / 1000 || 0
  if (seconds > 60) {
