...
Utils.setServiceDefaults = function (params, query) {
  ...
  if (query.geometry) query.geometry = coerceSRID(geometry)
  return query
}
...
function coerceSRID (geometry) {
  ...
  if (sr.wkt && sr.wkt.match(/WGS_1984_Web_Mercator_Auxiliary_Sphere/i)) {
...
Utils.createExportOptions = function (req, table, job, format) {
  ...
  } else if (req.query.geometry) {
    try {
      geometry = JSON.parse(req.query.geometry)
    } catch (e) {
      var coords = req.query.geometry.split(',')
      geometry = {
        xmin: coords[0],
        ymin: coords[1],
        xmax: coords[2],
