...
Utils.createCacheKey = function (params, query, legacy) {
  ...
  var toHash = params.item + '_' + (params.layer || 0) + JSON.stringify(sortedQuery)
...
Utils.createName = function (item, service, layerId) {
  ...
  var singleLayerItem = urlParts[urlParts.length - 1].match(/\d+/) || datasets.length < 2
  var layerName
...
Utils.cleanseName = function (name) {
  // replace "/", ",", "&", "|", "(", ")"
  return name.replace(/\/|,|&|\||\(|\)|\>|\<|\+|\-|\0/g, '').replace(/\s/g, '_') // eslint-disable-line
}
...
Utils.createExportOptions = function (req, table, job, format) {
  ...
  return {
    ...
    outSR: Utils.scrubSR((job && job.where) || (req.query && req.query.outSR)),
...
function setWhere (job, query) {
  var where = (job && job.where) || (query && query.where)
