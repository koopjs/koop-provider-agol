...
Utils.createCacheKey = function (params, query) {
  ...
  if (params.format !== 'zip' && params.format !== 'shp') delete cacheParams.outSR
  if (emptyQuery(cacheParams)) return 'full'
...
Utils.createCacheOptions = function (req, job) {
  job = job || {}
  var key = job ? 'full' : req.optionKey
...
Utils.createExportOptions = function (req, table, job, format) {
  ...
  var layer = (job && job.layer) || req.params.layer
  var fileName = item + '_' + layer + '.' + (format || req.params.format)
  var geojson = item + '_' + layer + '.geojson'
  var sourceDirname = Utils.createFilePath('full', job || req.params)
  var outputDirname = Utils.createFilePath(req.optionKey, job || req.params)
  var geometry
  if (job && job.geometry) geometry = job.geometry
  else if (req.query.geometry) geometry = JSON.parse(req.query.geometry)
  return {
    item: item,
    layer: layer,
