...
function getLayer (candidate) {
  ...
  } else if (candidate.match(/\.csv|\.kml|\.zip|\.geojson|\.json|\.shp|\.png|\.geohash/)) {
    parts = candidate.split('.')
    layer = parts[0]
  }
...
Utils.createExportOptions = function (req, table, job, format) {
  var fileName = table.name + '.' + (format || req.params.format)
  var geojson = table.name + '.geojson'
  var item = (job && job.item) || req.params.item
  var layer = (job && job.layer) || req.params.layer
  var sourceDirname = Utils.createFilePath('full_' + layer, job || req.params)
