...
Utils.createExportOptions = function (req, table) {
  return {
    table: 'agol' + ':' + req.params.item + ':' + req.params.layer || 0,
 ...
 * @return {bool} whether or not the particular file is generating
 */
Utils.isGenerating = function (req, info) {
  var generating = info.generating || {}
  var genKey = generating[req.optionKey] || {}
  var genFmt = genKey[req.params.format]
  if (['queued', 'start', 'progress'].indexOf(genFmt) > -1) return true
  else return false
}
