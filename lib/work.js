...
Work.prototype.importService = function (job, callback) {
  ...
  .on('done', function (result) {
    ...
    self.updateQueue.push({key: key, info: result.info, cache: self.cache}, function (err) {
      ...
      if (result.updated && config.api) self.enqueueExports(job)
      callback()
    })
  })
  .run()
...
Work.prototype.enqueueExports = function (job) {
  var self = this
  self.log.debug('Enqueing exports', job)
  self.exporter.bulk({}, [job], function (err) {
    if (err) self.log.error(err)
  })
}
